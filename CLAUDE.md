...
- Title search (`src/lib/search/`) matches `f_unaccent(title)` / `f_unaccent(original_title)` with pg_trgm. `f_unaccent` is an IMMUTABLE wrapper (migration 0012), because `unaccent()` itself can't be used in an index. Queries must use the same expressions as the GIN indexes on `titles`, or Postgres falls back to a seq scan. Results rank by text match, then `quality`, not `popularity` (which isn't comparable across sources).
- A user's region is stored only on their `user_services` rows (there's no users table), so saving zero services also forgets the region. Treat "no services" as "no availability filter". `user_titles.user_id` / `user_services.user_id` are Clerk user ids. Services count only `flatrate`/`free`/`ads` providers, not rent/buy stores.
- The onboarding deck (`src/lib/onboarding/deck.ts`) clusters the top 150 titles per type (by popularity percentile within each quality pool). It uses embeddings only when every pooled title has one; otherwise it falls back to genre multi-hot vectors, because the two can't be mixed. The deck is seeded from the user id, so it stays the same across reloads.
