...
// Pages already carry full records, so every title is upserted (or aliased to its show);
// resume with startPage.
export async function ingestAniListPages(
  ...
  for (let page = opts.startPage; page <= lastPage; page++) {
    ...
    await Promise.all(
      media.map(async (m) => {
        try {
          counts[await upsertWithAnimeMapping(db, mapAniListMedia(m))]++;
        } catch (err) {
          counts.failed++;
